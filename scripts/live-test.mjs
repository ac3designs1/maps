...
try {
  // Static
  {
    ...
    if (status === 200 && text.includes("app.js?v=43") && text.includes("styles.css?v=43") && text.includes("vendor/leaflet.js") && !text.includes("unpkg.com/leaflet")) ok("GET /", "cache v=43 local leaflet");
    else fail("GET /", "status " + status);
  }
  {
    const { status, text } = await req("/styles.css?v=43");
  ...
  {
    const { status, text } = await req("/app.js?v=43");
    ...
    const guards = ["function backToList", "lockStart:isHereStop(S.trip.stops[0])", "Where to?", "readOnly", "is-hot", "function updateHereDot", "You're offline.", "function forgetLast", "isHereStop(stop) && !hit.here", "smoothFactor:0", "startTrafficWatch", "trafficDelayS", "maps.recentPlaces", "function runSuggest", "function cancelSuggest"];
  ...
  // IDs in HTML exist in JS
  {
    ...
    const js = (await req("/app.js?v=43")).text;
