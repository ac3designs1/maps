...
try {
  // Static
  {
    ...
    if (status === 200 && text.includes("app.js?v=60") && text.includes("styles.css?v=60") && text.includes("vendor/leaflet.js") && !text.includes("unpkg.com/leaflet")) ok("GET /", "cache v=60 local leaflet");
    else fail("GET /", "status " + status);
  }
  {
    const { status, text } = await req("/styles.css?v=60");
  ...
  {
    const { status, text } = await req("/app.js?v=60");
  ...
  // IDs in HTML exist in JS
  {
    ...
    const js = (await req("/app.js?v=60")).text;
