...
try {
  // Static
  {
    ...
    if (status === 200 && text.includes("app.js?v=67") && text.includes("styles.css?v=67") && text.includes("vendor/leaflet.js") && !text.includes("unpkg.com/leaflet") && text.includes("black-translucent") && text.includes("bootSplash") && text.includes("interactive-widget") && text.includes("format-detection") && text.includes("tabBar") && text.includes("homeWhere")) ok("GET /", "cache v=67 local leaflet");
    else fail("GET /", "status " + status);
  }
  {
    const { status, text } = await req("/styles.css?v=67");
  ...
  {
    const { status, text } = await req("/app.js?v=67");
  ...
  }
  {
    ...
    if (status === 200 && typeof json?.adsenseClient === "string" && typeof json?.adsenseSlot === "string") ok("GET /api/config");
  ...
  // IDs in HTML exist in JS
  {
    ...
    const js = (await req("/app.js?v=67")).text;
