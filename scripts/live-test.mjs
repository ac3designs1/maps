...
try {
  // Static
  {
    ...
    if (status === 200 && text.includes("app.js?v=77") && text.includes("styles.css?v=77") && text.includes("vendor/leaflet.js") && !text.includes("unpkg.com/leaflet") && text.includes("black-translucent") && text.includes("bootSplash") && text.includes("interactive-widget") && text.includes("format-detection") && text.includes("tabBar") && text.includes("homeWhere") && text.includes("ca-pub-6171482823199192")) ok("GET /", "cache v=77 local leaflet");
    else fail("GET /", "status " + status);
  }
  {
    const { status, text } = await req("/styles.css?v=77");
    if (status === 200 && text.includes(".screen-planner") && text.includes("pointer-events:none") && text.includes(".action-chip.is-hot") && text.includes(".mrow.checked") && text.includes("pinch-zoom") && text.includes("calc(100% - 58px") && text.includes("var(--kb)") && text.includes(".sug-row.is-on") && text.includes(".dwell-btn") && text.includes(".trip-dup") && text.includes(".trip-del-reveal") && text.includes(".btn-danger") && text.includes(".stop-row.is-done") && text.includes(".install-hint") && text.includes("html.is-standalone") && text.includes(".sug-skel") && text.includes("100dvh") && text.includes("#bootSplash") && text.includes("user-select:none") && text.includes(".tab-bar") && text.includes(".home-search") && text.includes(".ios-switch") && text.includes(".header-add") && text.includes(".suggest-box") && text.includes("position:fixed") && text.includes("is-typing") && text.includes("overflow-anchor:none")) ok("GET styles.css");
    else fail("GET styles.css", "status " + status);
  }
  {
    const { status, text } = await req("/app.js?v=77");
  ...
  {
    const { status } = await req("/icon-192.png");
    if (status === 200) ok("GET icon-192.png");
    else fail("GET icon-192.png", "status " + status);
  }
  {
    const { status, text } = await req("/sw.js");
    if (status === 200 && text.includes("trips-v77")) ok("GET sw.js");
    else fail("GET sw.js", "status " + status);
  }
  {
    const { status, text } = await req("/vendor/leaflet.js?v=77");
    if (status === 200 && text.length > 10000) ok("GET vendor leaflet.js", text.length + " bytes");
    else fail("GET vendor leaflet.js", "status " + status);
  }
  {
    const { status, text } = await req("/vendor/leaflet.css?v=77");
  ...
  }
  {
    ...
    if (status === 200 && text.includes("display_override") && text.includes("New trip") && text.includes("launch_handler")) ok("GET manifest");
  ...
  }
  {
    ...
    if (status === 200 && json?.adsenseClient === "ca-pub-6171482823199192" && typeof json?.adsenseSlot === "string") ok("GET /api/config");
    else fail("GET /api/config", "status " + status);
  }
  ...
  }
  {
    ...
    if (status === 200 && hits.length && !junk.length) ok("suggest rubber company", blob(hits[0]).slice(0, 60));
    else fail("suggest rubber company", junk.slice(0, 3).map(h => h.label).join(" | ") || "status " + status + " hits " + hits.length);
  }
  {
  ...
  // IDs in HTML exist in JS
  {
    ...
    const js = (await req("/app.js?v=77")).text;
    const ids = [...html.matchAll(/id="([^"]+)"/g)].map(m => m[1]);
    const missing = ids.filter(id => !js.includes('"' + id + '"') && !js.includes("'" + id + "'") && !["mapToggleIcon","mapToggleLabel","navTitle","navSub","continueTitle","continueSub","installTitle","installSub","iosShareWord","tabHome","tabTrips","tabSettings","homeGoHomeSub","homeGoWorkSub","homeRecentsHead","launchAdSlot","launchAdContinue"].includes(id));
    // map/list structural ids that JS must touch
    const required = ["listScreen","tripScreen","sheet","stopList","btnStart","btnBack","btnNew","btnEmptyNew","modal","toast","suggestBox","summaryTap","tabBar","homeWhere","settingsBody","launchAd"];
